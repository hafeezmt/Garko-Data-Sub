...
export default function TransactionTable({ transactions = [], isAdmin = false }) {
  ...
  const getNetworkBadge = (network) => {
    ...
    if (net.includes('mtn')) bg = 'bg-amber-100 text-amber-800 font-bold border border-amber-300';
    else if (net.includes('airtel')) bg = 'bg-rose-100 text-rose-800 font-bold border border-rose-300';
    else if (net.includes('glo')) bg = 'bg-emerald-100 text-emerald-800 font-bold border border-emerald-300';
    else if (net.includes('9mobile') || net.includes('etisalat')) bg = 'bg-green-100 text-green-800 font-bold border border-green-300';
    else if (net.includes('paystack')) bg = 'bg-sky-100 text-sky-800 font-bold border border-sky-300';
      ...
      <div className="p-4 sm:p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-accent focus:bg-white transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          <select
            ...
            className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-accent"
          >
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-100 text-slate-500 text-xs uppercase tracking-wider font-semibold">
              <th className="py-3.5 px-4 sm:px-6">Date & Ref</th>
              {isAdmin && <th className="py-3.5 px-4">User</th>}
              <th className="py-3.5 px-4">Type</th>
              <th className="py-3.5 px-4">Network</th>
              <th className="py-3.5 px-4">Details / Phone</th>
                ...
                <tr key={tx.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  {isAdmin && (
                    <td className="py-3.5 px-4 whitespace-nowrap">
                      <div className="font-semibold text-slate-900">{tx.profiles?.full_name || 'Customer'}</div>
                  ...
                  <td className="py-3.5 px-4">
                    <div className="font-medium text-slate-800">{tx.plan || 'Standard Transaction'}</div>
                    <div className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                      <Phone className="w-3 h-3" /> {tx.phone_number || '-'}
                    </div>
                  </td>

                  {/* Amount */}
                  <td className="py-3.5 px-4 text-right whitespace-nowrap">
                    <span className={`font-bold ${tx.type === 'fund_wallet' ? 'text-emerald-600' : 'text-slate-900'}`}>
              ...
              <tr>
                <td colSpan={isAdmin ? 7 : 6} className="py-12 text-center">
                  <div className="max-w-xs mx-auto text-slate-400">
                    ...
                    <p className="text-slate-600 font-semibold text-base">No transactions found</p>
